...
```{r}
select(litters_df,group)
pull(litters_df,group) ## this is hardly used, gets a vector
```
...
look for more than two pups alives
...
mutate(
  ...
  group = str_to_lower(group) ## str_to_lower() makes variables lowercase, replaces existing group variable
  )
```
...
```{r}
litters_df =
  read_csv("data_import_examples/FAS_litters.csv", na = c("NA",".","")) |> ## cmd shift m
  janitor::clean_names() |> 
